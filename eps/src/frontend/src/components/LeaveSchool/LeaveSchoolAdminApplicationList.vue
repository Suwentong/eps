<template>
    <div style="display: inline-block; width: 100%;">
        <h3 class="header">进出校门申请审核</h3>
        <p>申请总数: {{ leaveSchoolArray.length }}</p>
        ...
        <table class="content-table table table-striped" id="example">
            ...
            <tbody v-else v-for="leaveApplication in paginatedData" :key="leaveApplication.id" style="text-align: center;">
                ...
                <td v-else-if="leaveApplication.applicationType == 'enter'">进校申请</td>
...
<script>
    ...
    export default {
        name: "LeaveSchoolAdminApplicationList",
        props:{
            data: {
                type: Array,
                required: true,
                ...
                (response) => {
                    ...
                    for (let index = 0; index < this.leaveSchoolArray.length; index++) {
                        ...
                        if (this.leaveSchoolArray[index].status == 0){ 
                            this.notReviewed++;
                            this.leaveSchoolArray[index].frontendStatus = "等待审核";
                        }
                        else {
                            this.reviewed++;
                            this.leaveSchoolArray[index].frontendStatus = "已审核";
                        }
        ...
        methods: {
            review(submitDate, studentId) {
                ...
                this.$router.push( { path: "/leave-school-admin-application-list/review" } );
            },
            nextPage() {
